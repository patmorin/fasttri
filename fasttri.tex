...
\begin{itemize}
  ...
  \item There exists an algorithm that, given an $n$-vertex planar graph $G$ and an integer $p$, runs in $O(n\log n)$ time and computes $p$-centered colouring of $G$ using at most $3(p+1)\binom{p+3}{3}$ colours \cite{debski.felsner.ea:improved}.
\end{itemize}
