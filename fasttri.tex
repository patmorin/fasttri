...
An interval splitting data structure stores a subset $S$ of $\{1,\ldots,n\}$ under the following two operations, each of which takes an integer argument $x\in\{1,\ldots,n\}$:
\begin{itemize}
    \item $\textsc{Add}(x)$: Add $x$ to the set $S$, i.e., $S\gets S\cup \{x\}$.
    \item $\textsc{Interval}(x)$: Return the pair $(i,j)$ where $i=\max\{y\in S\cup\{0\}:y<x\}$ and $j=\min\{y\in S\cup\{n+1\}:y\ge x\}$.
\end{itemize}

\begin{lem}
    There exists a data structure that preprocesses an integer $n$ and supports the $\textsc{Add}(x)$ and $\textsc{Interval}(x)$ operations.  The data structures uses $O(n)$ preprocessing time and any sequence of $m$ operations on this data structure runs in $O(m+n\log n)$ time.
\end{lem}

\begin{proof}
    The data structure consists of an array $a_1,\ldots,a_n$ of pointers.  The array entry $a_x$ points to a memory location $m$ storing the interval $(i,j)$ that answers the $\textsc{Interval}(x)$ query.  In this way each $\textsc{Interval}(x)$ query operation runs in $O(1)$ time.

    More specifically, suppose the elements $x_1<\cdots<x_k$ have been added to the data structure by calls to the $\textsc{Add}(x)$ operation.  Let $x_0:=0$ and $x_{k+1}=n+1$.  Then, for each $i\in \{0,\ldots,k\}$, the array locations $a_{x_{i}+1},\ldots,a_{x_{i+1}}$ all point the same memory location $m$ that contains the pair $(i,j)$ that answers the $\textsc{Interval}(x)$ query for any value $x\in\{x_{i}+1,\ldots,x_{i+1}\}$.

    To perform an $\textsc{Add}(x)$ operation, one first looks at the pair $(i,j)$ stored at the memory location $m$ referenced by $a_x$.
    Observe that the $j-i$ array entries $a_{i+1},\ldots,a_{j}$ all point to the same memory location $m$ and let $k:=x-i$.  The data structure allocates a new memory location $m'$ containing a pair $(i',j')$.  The algorithm then does the smallest amount of work possible, as follows:

    \begin{enumerate}
        \item If $k\le (j-i)/2$, then it sets $(i',j')\gets (i,x)$, sets $(i,j)\gets (x,j)$ and sets $a_{i+1},\ldots,a_{x}\gets m'$.
        \item Otherwise, $k> (j-i)/2$, then it sets $(i',j')\gets (x,j)$, sets $(i,j)\gets (i,x)$ and sets $a_{x+1},\ldots,a_{j}\gets m'$.
    \end{enumerate}
    It is straightforward to verify that these operations are correct.  To analyze the running time, we use the potential method of amortized analysis.  For each $x\in\{1,\ldots,n\}$, let $\Phi_x=c\log_2(j-i)$ where $(i,j)$ is the answer to $\textsc{Interval}(x)$ and let $\Phi=\sum_{x=1}^n \Phi_x$.  Observe that $0\le \Phi_x\le c\log_2(n+1)$ for each $x\in\{1,\ldots,n\}$, so $0\le \Phi\le cn\log_2(n+1)$.

    When an $\textsc{Add}(x)$ operation runs, it updates some number $z$ of array entries (either $z=k$ or $z=j-i-k$.   This takes $O(z)$ time, but causes $\Phi_\ell$ to decrease by at least $c$ for each array entry $a_\ell$ that has changed and does not increase $\Phi_\ell$ for any value of $\ell$.  Therefore, letting $\Phi$ and $\Phi'$ denote the value of $\Phi$ before and after this operation, we have $\Phi'-\Phi \le -cz$.  The amortized running time of this operation is therefore $O(1+z+\Phi'-\Phi) = O(1)$ for a sufficiently large constant $c$.

    Each $\textsc{Interval}(x)$ operation runs in $O(1)$ time and has no effect on $\Phi$, so it runs in $O(1)$ amortized time.  Therefore each operation runs in $O(1)$ amortized time, the minimum potential is $0$ and the maximum potential is $cn\log_2(n+1)$, so the total running time of any sequence of $m$ operations is at most $O(m + n\log n)$.
\end{proof}

\subsection{Nearest Marked Ancestor}

We require a data structure that stores a (BFS) tree $T$ and maintains a set $S\subset V(T)$ such that $T[S]$ is connected and, if non-empty, $S$ contains the root of $T$.  Initially the set $S$ contains only the root of $T$.  We must support the following operations:

\begin{itemize}
    \item $\textsc{Mark}(v)$: Add $w$ to the set $S$.  A precondition of this operation is that the parent of $v$ of $w$ is already in $S$ but $w$ is not yet in $S$.

    \item $\textsc{NearestMarkedAncestor}(v)$:  Return the first node $u\in S$ that is on the path from $v$ to the root of $T$.
\end{itemize}

\begin{lem}
    There exists a data structure that supports the $\textsc{Mark}(v)$ and $\textsc{NearestMarkedAncestor}(v)$ operations on an $n$-vertex tree $T$. This data structure requires $O(n)$ preprocessing time and can perform any sequence of $m$ operations in $O(m + n\log n)$ time.
\end{lem}

\begin{proof}
    Let $T'$ be the directed graph obtained by replacing each undirected edge $vw$ of $T$ with two directed edges $vw$ and $wv$.  Since every vertex of $T'$ has the same in and out degree, it is Eulerian.  Let $v_0,e_1,v_2,e_3,\ldots,v_{3n-3}$ be the sequence of vertices and edges encountered during an Euler tour of $T'$ that begins and ends at the root $v_0=v_{3n+3}$ of $T$.  Observer that if some edge $e$ of $T$ maps to two edges $e_i=vw$ and $e_j=wv$ with $i<j$, then $v_{i+1}=v_{j-1}=w$ and $v_{i+1},\ldots,v_{j-1}$ contains every edge and vertex in the subtree of $T$ rooted at $w$.  In this way, each edge $vw$ of $T$ joining a node $w$ to its parent $v$ maps to an interval $(i_{vw},j_{vw})$.  Similarly, each node $v$ in $T$ maps to an interval $(i_v,j_v)$ where $i_v=\min\{i:v_i=v\}$ and $j_v=\max\{j:v_j=v\}$.

    The data structure will therefore store a subset $S'$ of the integers $\{1,\ldots,2n-2\}$ so it can answer $\textsc{Interva}(x)$ queries on $S'$.  In particular, for each edge $vw$ of $T$ where the parent $v$ of $w$ is contained in $S$, the data structure will store the integers $i_{vw}$ and $j_{vw}$.  In this way, the total number of $\textsc{Add}(x)$ operations resulting from any sequence of $\textsc{Mark}(v)$ operations is at most $2n-2$, the number of edges in $T$.

    Along with each node $v\in V(T)$ we store integers $i_v=\min\{i:v_i=v\}$ and $j_v=\max\{j:v_j=v\}$.  To answer a $\textsc{NearestMarkedAncestor}(u)$ query, we perform two $\textsc{Interval}(x)$ queries with the values $x:=i_v$ and $x:=j_v$.  There are two cases to consider:
    \begin{enumerate}
        \item If $u\not\in S$, then this both queries will return the same value $(i,j)$ such that $e_i=vw$, $e_j=wv$, $i< i_u \le j_u < j$ and $v$ is the nearest marked ancestor of $u$.

        \item If $u\in S$, then this query will return a value $(i,j)$ such that $i< i_v$
